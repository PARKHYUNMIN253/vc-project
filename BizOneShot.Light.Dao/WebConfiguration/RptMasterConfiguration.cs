...
namespace BizOneShot.Light.Dao.WebConfiguration
{
    ...
    internal partial class RptMasterConfiguration : EntityTypeConfiguration<RptMaster>
    {
        ...
        public RptMasterConfiguration(string schema)
        {
            ...
            HasKey(x => new { x.QuestionSn, x.NumSn, x.BasicYear });
            ...
            // Foreign keys
            HasOptional(a => a.VcUsrInfo).WithMany(b => b.RptMasters).HasForeignKey(c => c.MentorId); // FK_SC_USR_TO_RPT_MASTER
            HasRequired(a => a.QuesMaster).WithMany(b => b.RptMasters).HasForeignKey(c => c.QuestionSn); // FK_QUES_MASTER_TO_RPT_MASTER
            HasRequired(a => a.VcCompInfo).WithMany(b => b.RptMasters).HasForeignKey(c => c.CompSn); // FK_SC_COMP_INFO_TO_RPT_MASTER
            InitializePartial();
        }
