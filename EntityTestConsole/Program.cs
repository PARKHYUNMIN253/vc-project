...
namespace EntityTestConsole
{
    ...
    class Program
    {
        static  void Main(string[] args)
        {
            ...
            //using (var context = new WebDbContext())
            //{
            //    //var listRptMatsers = context.VcCompInfoes.Where(cm => cm.CompSn == 94).Select(sm => sm.VcUsrInfoes).Select(tt => tt.RptMasters).ToList();
            //    var listRptMatsers = context.VcCompInfoes.Where(cm => cm.CompSn == 94).Select(sm => sm.VcUsrInfoes).ToList();
        ...
        public static void UpdateScCompInfo()
        {
            using (var db = new WebDbContext())
            {
                var compinfo = db.VcCompInfoes.Find(6);
                if (compinfo != null)
                {
        ...
        public static  void DeleteCompInfo()
        {
            using (var db = new WebDbContext())
            {
                var compinfo = db.VcCompInfoes.Find(30);

                db.VcCompInfoes.Remove(compinfo);
