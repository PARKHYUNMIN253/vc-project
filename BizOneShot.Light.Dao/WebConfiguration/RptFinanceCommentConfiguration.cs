...
namespace BizOneShot.Light.Dao.WebConfiguration
{
    ...
    internal partial class RptFinanceCommentConfiguration : EntityTypeConfiguration<RptFinanceComment>
    {
        ...
        public RptFinanceCommentConfiguration(string schema)
        {
            ...
            // Foreign keys
            HasRequired(a => a.VcCompInfo).WithMany(b => b.RptFinanceComments).HasForeignKey(c => c.CompSn); // FK_SC_COMP_INFO_TO_RPT_FINANCE_COMMENT
            HasRequired(a => a.VcUsrInfo).WithMany(b => b.RptFinanceComments).HasForeignKey(c => c.ExpertId); // FK_SC_USR_TO_RPT_FINANCE_COMMENT
            InitializePartial();
        }
