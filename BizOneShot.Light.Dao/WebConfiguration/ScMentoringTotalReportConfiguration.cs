...
namespace BizOneShot.Light.Dao.WebConfiguration
{
    ...
    internal partial class ScMentoringTotalReportConfiguration : EntityTypeConfiguration<ScMentoringTotalReport>
    {
        ...
        public ScMentoringTotalReportConfiguration(string schema)
        {
            ...
            Property(x => x.DeepenContents).HasColumnName("DEEPEN_CONTENTS").IsOptional().HasColumnType("nvarchar").HasMaxLength(1000);
            Property(x => x.FinalSubmitYn).HasColumnName("FINAL_SUBMIT_YN").IsOptional().IsFixedLength().IsUnicode(false).HasColumnType("char").HasMaxLength(1);
            Property(x => x.ConCode).HasColumnName("CON_CODE").IsOptional().HasColumnType("nvarchar").HasMaxLength(5);

            // Foreign keys
            HasOptional(a => a.VcCompInfo).WithMany(b => b.ScMentoringTotalReports).HasForeignKey(c => c.CompSn); // FK_SC_COMP_INFO_TO_MENTORING_TOTAL_REPORT
            HasOptional(a => a.VcUsrInfo).WithMany(b => b.ScMentoringTotalReports).HasForeignKey(c => c.MentorId); // FK_SC_USR_TO_MENTORING_TOTAL_REPORT
            InitializePartial();
        }
