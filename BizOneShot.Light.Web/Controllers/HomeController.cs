...
namespace BizOneShot.Light.Web.Controllers
{
    ...
    public class HomeController : BaseController
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel loginViewModel)
        {
            ...
            string cookieId = Request.Cookies["tcms_login_key"].Value;  // tcms login key 받아오기 성공

            VcUsrInfo scUsr = await _scUsrService.SelectScUsr(loginViewModel.ID.Trim());    // tcms login key로 db에 조회해서 있으면, 없으면으로 
            if (scUsr != null)
            {
                //패스워드비교
                SHACryptography sha2 = new SHACryptography();
        ...
        {
            var scUsers = await _vcCompInfoService.getVcCompInfoForSearchId(USR_NO);
        ...
        public ActionResult HomeConfirm()
        {
            return View();
        }
