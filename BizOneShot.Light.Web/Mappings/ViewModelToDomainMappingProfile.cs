...
namespace BizOneShot.Light.Web.Mappings
{
    public class ViewModelToDomainMappingProfile : Profile
    {
        ...
        protected override void Configure()
        {
            ...
            //전문가 회원가입 to 회원
            Mapper.CreateMap<JoinExpertViewModel, VcUsrInfo>()
            ...
            //전문가 회원가입모델 to 회사
            Mapper.CreateMap<JoinExpertViewModel, VcCompInfo>()
                .ForMember(d => d.OwnTelNo, map => map.MapFrom(s => s.ComTelNo1 + "-" + s.ComTelNo2 + "-" + s.ComTelNo3))
            ...
            //멘토 회원가입 to 회원
            Mapper.CreateMap<JoinMentorViewModel, VcUsrInfo>()
                .ForMember(d => d.Email, map => map.MapFrom(s => s.Email1 + "@" + s.Email2));

            //멘토 회원가입모델 to 회사
            Mapper.CreateMap<JoinMentorViewModel, VcCompInfo>();
            ...
            //회원가입모델 to 회원
            Mapper.CreateMap<JoinCompanyViewModel, VcUsrInfo>()
                .ForMember(d => d.TelNo, map => map.MapFrom(s => s.TelNo1 + "-" + s.TelNo2 + "-" + s.TelNo3))
                .ForMember(d => d.MbNo, map => map.MapFrom(s => s.MbNo1 + "-" + s.MbNo2 + "-" + s.MbNo3))
                .ForMember(d => d.FaxNo, map => map.MapFrom(s => s.FaxNo1 + "-" + s.FaxNo2 + "-" + s.FaxNo2))
                .ForMember(d => d.Email, map => map.MapFrom(s => s.Email1 + "@" + s.Email2));
                //.ForMember(d => d.UseErp, map => map.MapFrom(s => s.UseErp.ToString()));

            //회원가입모델 to 회사
            Mapper.CreateMap<JoinCompanyViewModel, VcCompInfo>()
                .ForMember(d => d.OwnTelNo, map => map.MapFrom(s => s.ComTelNo1 + "-" + s.ComTelNo2 + "-" + s.ComTelNo3))
                //.ForMember(d => d.Addr1, map => map.MapFrom(s => s.ComAddr1))
                //.ForMember(d => d.Addr2, map => map.MapFrom(s => s.ComAddr2))
                //.ForMember(d => d.PostNo, map => map.MapFrom(s => s.ComPostNo))
            ...
            //사업관리 담당자 to 회원
            Mapper.CreateMap<BizWorkViewModel, VcUsrInfo>()
                .ForMember(d => d.TelNo, map => map.MapFrom(s => s.TelNo1 + "-" + s.TelNo2 + "-" + s.TelNo3))
                .ForMember(d => d.MbNo, map => map.MapFrom(s => s.MbNo1 + "-" + s.MbNo2 + "-" + s.MbNo3))
            ...
            //사업관리 담당자 to 사업
            Mapper.CreateMap<BizWorkViewModel, VcBizWork>();
            ...
            ////중소기업평균 등록
            Mapper.CreateMap<RegCompanyAverageViewModel, ScCav>();
            
            //상품화 시장역량 등록
            Mapper.CreateMap<RegMarketStateViewModel, ScMak>();

            // 재무데이터 입력부분 1월 29일 추가
            Mapper.CreateMap<FinanceCompositionViewModel, ScCompanyFinance>();

            //모든 문서 status
            Mapper.CreateMap<TcmsCompStatusSelectViewModel, TcmsCompStatusSelectView>();
            //TCMS 관리자 보고서 화면
            Mapper.CreateMap<BasicSurveyReportViewModel, TcmsCompStatusSelectView>();
