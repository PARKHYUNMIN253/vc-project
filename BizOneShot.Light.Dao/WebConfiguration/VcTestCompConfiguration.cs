...
{
    // VC_TEST_COMP
    internal partial class VcTestCompConfiguration : EntityTypeConfiguration<VcTestComp>
    {
        public VcTestCompConfiguration()
            : this("dbo")
        {
        }
 
        public VcTestCompConfiguration(string schema)
        {
            ToTable(schema + ".VC_TEST_COMP");
            HasKey(x => x.CompSn);
