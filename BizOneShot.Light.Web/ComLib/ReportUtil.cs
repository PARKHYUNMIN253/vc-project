...
namespace BizOneShot.Light.Web.ComLib
{
    public class ReportUtil
    {
        ...
        public ReportUtil(IScBizWorkService scBizWorkService, IQuesResult1Service quesResult1Service, IQuesResult2Service quesResult2Service, IQuesMasterService quesMasterService, ISboFinancialIndexTService sboFinancialIndexTService)
        {
            this.scBizWorkService = scBizWorkService;
            this.quesResult1Service = quesResult1Service;
            this.quesResult2Service = quesResult2Service;
        ...
        public async Task<double> GetOverAllManagementTotalPoint(int questionSn)
        {
            ...
            // 나머지는 총점에 계산x( 1.이직직원수 존재 && 총직원 0, 2. 이직직원 수 0 && 총직원 0)

            //// 만약 총 직원의 수가 0 이거나(or) 이직 직원의 수가 0이면
            //if (double.Parse(totalEmp.D451) == 0 && double.Parse(moveEmp.D451) == 0)
            //{

            //}
            //else if (double.Parse(totalEmp.D451) == 0 || double.Parse(moveEmp.D451) == 0)    // (**2.26 산식 수정**) 총 직원과 이직직원 각각이 0일때 재무점수 0으로 고정
            //{
            //    avg = 0.0;  // 평균 값은 0
            //}
            //else // (**2.26 산식 수정**) 둘 다 0이 아닐 때만 재무점수 계산
            //{
            //    avg = (int.Parse(moveEmp.D451) / double.Parse(totalEmp.D451)) * 100;
            //    totalPoint = totalPoint + ReportHelper.CalcPoint(ReportHelper.GetCodeTypeD(avg), 3);
            //}
